...
- **Weather fetch retries** — `_fetch_with_retry()` in `scheduler.py` retries up to 3 times with a 30s delay; constants are `WEATHER_FETCH_RETRIES` and `WEATHER_FETCH_RETRY_DELAY`
